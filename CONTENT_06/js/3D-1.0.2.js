...
function simulateMouse(event, touch) {
...
function touchHandler(event, ignoreEls) {
    ...
    if (event.type === "touchmove") {
        ...
        if (!touch) return;
        return void simulateMouse(event, touch);
    }

    if (event.type === "touchend" || event.type === "touchcancel") {
        const touch = [...event.changedTouches].find((t) => t.identifier === activeTouchId);
        if (!touch) return;
        simulateMouse(event, touch);
        activeTouchId = null;
        return;
    }
...
function isDragControlActive() {
    if (g_isDragging) return true;
    if (!g_latestMousePress || !g_state_controls) return false;
    let ctrl = g_state_controls.find((c) => c.id == g_latestMousePress || c.class == g_latestMousePress || c.name == g_latestMousePress);
    return !!(ctrl && (ctrl.type === "drag" || ctrl.type === "scrollbar"));
}
window.isDragControlActive = isDragControlActive;

function isDragElement(el) {
    if (!el || !g_state_controls) return false;
    let $el = $(el);
    return g_state_controls.some((c) => {
        if (c.type !== "drag" && c.type !== "scrollbar") return false;
        if (c.id && ($el.attr("id") === c.id || $el.closest(`#${c.id}`).length > 0)) return true;
        if (c.class && ($el.hasClass(c.class) || $el.closest(`.${c.class}`).length > 0)) return true;
        if (c.class_name && ($el.hasClass(c.class_name) || $el.closest(`.${c.class_name}`).length > 0)) return true;
        return false;
    });
}
window.isDragElement = isDragElement;
...
function controlTouch() {
    ...
    $("#rect-overlay").on("mousedown", function (e) {
        e.stopPropagation();
        e.preventDefault();
    });

    $("#rect-overlay").on("mouseup", function (e) {
        e.stopPropagation();
        e.preventDefault();
    ...
    $(document).on("mousedown", function (e) {
        if (!$("#rect-overlay").length) return;
        let targetId = $(e.target).attr("id");
        if (targetId === "rect-overlay" || isDragControlActive() || isDragElement(e.target)) {
            return;
        }
...
window.controlTouch = controlTouch;
    ...
    setTimeout(async () => {
        ...
        const fnControlDrag = (ctrl) => {
            ...
            let fnDrag = (event, isSnap, eventName, isKeepCheckDist) => {
                ...
                //if (eventName == 'mouseup' && Math.pow(dx, 2) + Math.pow(dy, 2) < 5 ) return;
